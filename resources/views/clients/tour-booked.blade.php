            ...
            <div class="summary-section">
                ...
                @else
                    {{-- <button type="submit" class="booking-btn btn-cancel-booking {{ $hide }}">Hủy
                        ...
                        <form action="{{ route('cancel-booking') }}" method="POST" id="cancelForm" style="display: block; width: 100%;">
        @csrf
        {{-- Đưa các input hidden vào đây để gửi dữ liệu đi --}}
        <input type="hidden" name="bookingId" value="{{ $bookingId }}">
        <input type="hidden" name="tourId" value="{{ $tour_booked->tourId }}">
        <input type="hidden" name="quantity__adults" value="{{ $tour_booked->numAdults }}">
        <input type="hidden" name="quantity__children" value="{{ $tour_booked->numChildren }}">

        {{-- Nút hủy bây giờ nằm trong form riêng, không bị script thanh toán chặn nữa --}}
